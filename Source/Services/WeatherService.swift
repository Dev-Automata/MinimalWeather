...
class WeatherService {
    ...
    func setRequestLanguage(_ appLang: String) {
        lang = appLang
    }

    func getWeatherForCity(name city: String) {
        let _ = apiClient.send(request: GetWeatherForCity(name: city, lang: lang)) { data, error in
            self.handleResponse(data: data, error: error)
        }
    }

    func getWeatherForLocation(latitude: String, longitude: String) {
        let _ = apiClient.send(request: GetWeatherForLocation(latitude: latitude, longitude: longitude, lang: lang)) { data, error in
            self.handleResponse(data: data, error: error)
        }
    }
    ...
    private func handleResponse(data: Data?, error: Error?) {
        ...
        guard let data = data else {
            ...
            delegate?.didWeatherFailWithError(error: NSError(domain: errorDomain, code: errorCode, userInfo: errorInfo))
            return
        }
